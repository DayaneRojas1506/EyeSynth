#!/usr/bin/env python3
"""Entrena el modelo FAKE/REAL con backbone COCO (detección de objetos).

ResNet-50 cuyo backbone parte de los pesos COCO de Faster R-CNN (features
semánticas de objeto) y se fine-tunea como clasificador FAKE/REAL sobre
`dataset_new_model/`. Es el baseline "semántico" frente al NPR (alta frecuencia).

Uso (en WSL/Linux con torch+torchvision):
    python server/train_coco_fakereal.py
    python server/train_coco_fakereal.py --epochs 20 --lr 1e-4
    python server/train_coco_fakereal.py --from-scratch   # backbone aleatorio
...
({"FAKE": 0, "REAL": 1}), igual que los demás modelos, para que
...
from coco_model import build_coco_model, build_transform
...
OUT_CKPT   = _PROJECT_ROOT / "server" / "coco_fakereal.pth"  # checkpoint de salida
FROM_SCRATCH = False   # True: backbone aleatorio (sin pesos COCO)

# --- Entrenamiento ---
# El experimento anterior (backbone entero descongelado) memorizaba el train
# (acc 0.95) y val_acc se quedaba en ~azar (0.52): overfitting de manual con un
# ResNet-50 de 23M params sobre ~560 imágenes. Por eso ahora:
#   - FREEZE_BACKBONE: linear probe (solo se entrena fc). Mide la señal real de
#     las features COCO sin que el backbone memorice. Es el baseline honesto.
#   - DROPOUT + LABEL_SMOOTHING: regularizan la cabeza y frenan la sobreconfianza.
#   - Como solo se entrena fc, podemos subir el LR y permitir más épocas.
FREEZE_BACKBONE  = True   # linear probe (recomendado para baseline honesto)
DROPOUT          = 0.3    # dropout antes de fc (0=off)
LABEL_SMOOTHING  = 0.1    # suaviza las etiquetas en CrossEntropy (0=off)

EPOCHS       = 60       # tope; el early stopping suele cortar antes
BATCH_SIZE   = 16
OPTIMIZER    = "adam"    # "adam" o "sgd"
LR           = 1e-3      # más alto: solo entrenamos la cabeza fc
WEIGHT_DECAY = 1e-3
VAL_SPLIT    = 0.15
PATIENCE     = 10        # early stopping (0=off); más paciencia con LR alto
NUM_WORKERS  = 0
SEED         = 42
...
def parse_args():
    p = argparse.ArgumentParser(description="Entrena FAKE/REAL (backbone COCO).")
    ...
    p.add_argument("--from-scratch", action="store_true", default=FROM_SCRATCH,
                   help="Backbone aleatorio (sin pesos COCO).")
    # --freeze-backbone (default on) y --no-freeze-backbone para descongelar todo.
    p.add_argument("--freeze-backbone", dest="freeze_backbone",
                   action="store_true", default=FREEZE_BACKBONE,
                   help="Linear probe: entrena solo fc (recomendado).")
    p.add_argument("--no-freeze-backbone", dest="freeze_backbone",
                   action="store_false",
                   help="Descongela todo el backbone (riesgo de overfitting).")
    p.add_argument("--dropout", type=float, default=DROPOUT,
                   help="Dropout antes de fc (0=off).")
...
def main():
    ...
    # ---- Modelo: backbone COCO (o aleatorio con --from-scratch) ----
    if args.from_scratch:
        print("Backbone aleatorio (sin pesos COCO).")
        model = build_coco_model(pretrained=False,
                                 freeze_backbone=args.freeze_backbone,
                                 dropout=args.dropout)
    else:
        print("Inicializando backbone con pesos COCO (Faster R-CNN ResNet-50 FPN).")
        model = build_coco_model(pretrained=True,
                                 freeze_backbone=args.freeze_backbone,
                                 dropout=args.dropout)
    ...
    mode = "LINEAR PROBE (solo fc)" if args.freeze_backbone else "fine-tuning completo"
    print(f"Modo: {mode} | entrenables: {n_train_params:,} / {n_total_params:,} "
          f"| dropout: {args.dropout} | label_smoothing: {args.label_smoothing}")

    # El optimizador solo recibe los parámetros entrenables: con el backbone
    # congelado eso es solo fc, y por eso podemos usar un LR más alto.
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.freeze_backbone:
            # Backbone congelado: ponerlo en eval() para CONGELAR también las
            # stats de BatchNorm (si no, model.train() las seguiría actualizando
            # con cada batch y desestabilizaría un backbone que no se entrena).
            # Solo la cabeza fc queda en modo train.
            model.eval()
            model.fc.train()
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                "arch": "coco_resnet50_fakereal",
                # Config de la cabeza: la inferencia debe reconstruir el modelo
                # con el MISMO dropout para que las claves del state_dict casen
                # (con dropout>0, fc es un Sequential -> claves 'fc.1.*').
                "dropout": args.dropout,
                "freeze_backbone": args.freeze_backbone,
            }, args.out)
